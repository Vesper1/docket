...
function fileNameOf(completion: Pick<StepCompletion, 'planIdentity' | 'step'>): string {
	return `${completion.planIdentity.replace(':', '_')}--${completion.step}.json`;
}

function parseCompletion(value: unknown, name: string): Result<StepCompletion, DocketError> {
