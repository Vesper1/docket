 ...
 */
export function parseConfig(text: string): Result<DocketConfig, DocketError> {
	let raw: unknown;
	try {
...
function parseEnvironments(raw: unknown): Result<readonly EnvironmentConfig[], DocketError> {
...
function parseEnvironment(id: string, raw: unknown): Result<EnvironmentConfig, DocketError> {
...
function parseGates(raw: unknown, where: string): Result<readonly GateDefinition[], DocketError> {
...
function requireSafeExecutionName(
	name: string,
	kind: 'gate' | 'step',
	where: string,
...
function duplicateName(entries: readonly { readonly name: string }[]): string | undefined {
	const names = entries.map((entry) => entry.name);
	return names.find((name, index) => names.indexOf(name) !== index);
}

function parseTimeout(record: Record<string, unknown>, where: string): Result<number, DocketError> {
...
function asList(raw: unknown, where: string): Result<readonly unknown[], DocketError> {
...
function parseTests(raw: unknown, where: string): Result<TestSelection, DocketError> {
...
function asRecord(value: unknown, where: string): Result<Record<string, unknown>, DocketError> {
...
function invalid(problem: string, found?: string): DocketError {
