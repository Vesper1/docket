...
function planArtifacts(): PlanArtifacts {
...
function runArtifacts(overrides: Partial<RunArtifacts> = {}): RunArtifacts {
...
async function writeInto(artifacts: RunArtifacts) {
...
describe('reading untrusted run artifacts', () => {
	...
	test('rejects a passed verdict with no Salesforce validation', async () => {
		...
		expect(isErr(result) && result.error.code).toBe(ErrorCode.planMismatch);
	});
...
async function alteredValidation(
...
describe('the secret scan', () => {
	...
	test('an artifact with a secret in it is not written at all', async () => {
		...
		expect(isErr(result) && result.error.code).toBe(ErrorCode.secretInArtifact);
