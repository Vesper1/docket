...
function plan(overrides: Partial<PlanRequest> = {}): PlanArtifacts {
...
describe('the deletion policy', () => {
	...
	test('a deletion fails closed when the environment forbids it', () => {
		...
		expect(isErr(result) && result.error.code).toBe(ErrorCode.destructiveNotAllowed);
		expect(isErr(result) && result.error.message).toContain('ApexClass:Old');
	});
