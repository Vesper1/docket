...
import { errorOf } from '../lib/shared/result/testing/expect-result.ts';
...
const deployedRun = async (base: TreeSnapshot, head: TreeSnapshot): Promise<RunRecord> => {
...
describe('M11.5 later-change protection', () => {
	test('a later class edit stops rollback instead of overwriting it', async () => {
		...
		expect(errorOf(proposal).code).toBe('rollback_conflict');
		expect(errorOf(proposal).message).toContain('Foo.cls-meta.xml');
	});
});
...
describe('M11.6 compensating pull request', () => {
	...
	test('a repository that forbids Actions pull requests fails with what to do about it', async () => {
		...
		const fake = createFakeGitHub({
			...
			'POST /repos/acme/salesforce/git/refs': {
				status: 201,
				body: { ref: `refs/heads/${proposal.value.plan.branch}`, object: { sha: commitSha } },
			},
			'POST /repos/acme/salesforce/pulls': {
				status: 403,
		...
		expect(errorOf(created).code).toBe('github_failed');
		const message = isErr(created) ? created.error.message : '';
		expect(message).toContain('DOCKET_PR_TOKEN');
		// The branch is already pushed, so the message has to name it.
		expect(message).toContain(proposal.value.plan.branch);
	});
	...
	test('a target branch that moved after calculation causes no GitHub write', async () => {
		...
		expect(errorOf(created).code).toBe('rollback_conflict');
		expect(fake.requests()).toHaveLength(1);
	});
});

const commitLater = async (cwd: string): Promise<string> => {
	const env = {
...
const pullResponse = (branch: string, sha: string): Record<string, unknown> => {
	return {
		number: 77,
