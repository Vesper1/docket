...
export const render = (result: Result<CliData, DocketError>, format: OutputFormat): CliOutcome => {
 ...
 */
const exitCodeOf = (data: CliData): ExitCode => {
 ...
 */
const runSummary = (data: { readonly run: RunRecord; readonly directory: string }): string => {
	const { run } = data;
	const lines = [
 ...
 */
const encode = (payload: unknown): string => `${JSON.stringify(payload)}\n`;
