...
const recorded = async (options: { kind?: RunKind; status?: 'passed' | 'failed' } = {}) => {
...
describe('M11.1 rollback source selection', () => {
	...
	test('a failed or validation-only run cannot start rollback', async () => {
		...
		expect(errorOf(failedResult).code).toBe('rollback_source_invalid');
		...
		expect(errorOf(validationResult).code).toBe('rollback_source_invalid');
	});
	...
	test('an unknown schema is rejected before any inverse plan exists', async () => {
		...
		const selected = await selectRollbackSource(fixture.directory);
		expect(errorOf(selected).code).toBe('plan_mismatch');
	});
