 ...
 */
export const publishValidationCheck = async (
	client: GitHubClient,
	...
	return ok({
		id,
		name: nonEmptyText(body?.['name']) ?? VALIDATION_CHECK_NAME,
		headSha: nonEmptyText(body?.['head_sha']) ?? request.headSha,
		conclusion: nonEmptyText(body?.['conclusion']) ?? '',
 ...
 * out against it.
 */
export const publishStepCheck = async (
	client: GitHubClient,
	request: {
		readonly repository: string;
			readonly headSha: string;
...
export const completeStepCheck = async (
	client: GitHubClient,
	request: {
		readonly repository: string;
		readonly headSha: string;
...
export const findStepCompletionRuns = async (
	client: GitHubClient,
	request: {
		readonly repository: string;
		readonly headSha: string;
...
export const stepCheckName = (step: string): string => `docket/step/${step}`;
 ...
 */
export const findOriginatingRun = async (
	client: GitHubClient,
	repository: string,
	headSha: string,
	...
	const ours = checks.filter((check) => decodeExternalId(nonEmptyText(check['external_id'])) !== undefined);
	if (ours.length === 0) {
		...
		const failed = checks.find((check) => nonEmptyText(check['conclusion']) !== 'success');
		return err(
			failed === undefined
				? docketError(
				...
				: docketError(
						ErrorCode.validationNotPassed,
						`the ${VALIDATION_CHECK_NAME} check for ${headSha} concluded ${nonEmptyText(failed['conclusion']) ?? 'nothing'}`,
					),
		);
	}

	const passed = ours.find((check) => nonEmptyText(check['conclusion']) === 'success');
	if (passed === undefined) {
		return err(
			docketError(
				ErrorCode.validationNotPassed,
				`the ${VALIDATION_CHECK_NAME} check for ${headSha} concluded ${nonEmptyText(ours[0]?.['conclusion']) ?? 'nothing'}`,
			),
		);
	}

	const decoded = decodeExternalId(nonEmptyText(passed['external_id']));
 ...
 */
const encodeExternalId = (workflowRunId: string, planIdentity: string): string => {
	return JSON.stringify({ workflowRunId, planIdentity });
};

const decodeExternalId = (value: string | undefined): OriginatingRun | undefined => {
	if (value === undefined) return undefined;

	try {
		const parsed = asRecord(JSON.parse(value));
		const workflowRunId = nonEmptyText(parsed?.['workflowRunId']);
		const planIdentity = nonEmptyText(parsed?.['planIdentity']);
		return (
...
const matchingStepCheck = async (
	...
	for (const raw of Array.isArray(checks) ? checks : []) {
		...
		const identity = decodeStepCheck(nonEmptyText(check?.['external_id']));
...
const encodeStepCheck = (identity: StepCheckIdentity): string => {
...
const decodeStepCheck = (value: string | undefined): StepCheckIdentity | undefined => {
	if (value === undefined) return undefined;
	...
	try {
		...
		if (
			record?.['v'] !== 1 ||
			nonEmptyText(record['s']) === undefined ||
			nonEmptyText(record['p']) === undefined ||
			nonEmptyText(record['vr']) === undefined ||
			!(record['cr'] === null || nonEmptyText(record['cr']))
		) {
			return undefined;
		}
