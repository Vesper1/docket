...
export const readPullRequest = async (
	...
	const baseBranch = nonEmptyText(base?.['ref']);
	...
	return ok({
		repository,
		number,
		state: nonEmptyText(body['state']) ?? 'unknown',
		draft: body['draft'] === true,
		merged: body['merged'] === true,
		baseBranch,
		baseSha: baseSha.value,
		headSha: headSha.value,
		headRepository: nonEmptyText(asRecord(head?.['repo'])?.['full_name']) ?? '',
		mergeCommitSha,
	});
};
 ...
 */
export const requireValidatablePullRequest = (
 ...
 */
export const requireMergedPullRequest = (
	pullRequest: PullRequestFacts,
): Result<PullRequestFacts, DocketError> => {
	if (pullRequest.headRepository !== pullRequest.repository) {
...
const ineligible = (pullRequest: PullRequestFacts, reason: string): DocketError => {
	return docketError(
